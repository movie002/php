...
//	$DH_index_cat_each_ins = str_replace("%cat_each_title%","精彩预告",$DH_index_cat_each);
	...
	$sql="select * from page where DATE_SUB(CURDATE(), INTERVAL 2 MONTH) <= date(pubdate) and DATE_SUB(CURDATE(), INTERVAL 2 WEEK) <= date(updatetime) order by hot desc limit 0,9";
	$results=mysql_query($sql,$conn);	
	if($results)
	{
		$liout='<div class="titleClass">
					<div class="tprev"><div style="margin:14px 0">◄</div></div>
					<div class="tnext"><div style="margin:14px 0">►</div></div>
					<ul>';
		//$imgi=1;
		while($row = mysql_fetch_array($results))
		{
			...
			$lieach = "\n".'<li><a href="'.$htmlpath.'"><img class="imglarge" data-src="'.$imgurl.'" alt="'.$row['title'].'的海报" ><div style="text-align:center;color:#447fa8">'.$row['title'].'</div></a></li>';
			$liout.= $lieach;
			//$imgi++;
		}
		$liout.="\n".'</ul></div>';
	...
	$DH_index_cat_eachs .='<div style="width:50%;float:left">'.$liout.'</div>';
...
//	$DH_index_cat_each_ins = str_replace("%cat_each_title_more%","",$DH_index_cat_each);
	...
	//最上面右侧内容
	$liout ="\n<div class=\"TabTitle\"> \n <ul id=\"myTabt\">";
	$liout .="\n".'<li class="active" onclick="nTabs(this,0);">最近热门</li>';
	...
	$liout ='<div class="TabContent">';		
		...
		$sql="select * from page where DATE_SUB(CURDATE(), INTERVAL 2 MONTH) <= date(pubdate) and DATE_SUB(CURDATE(), INTERVAL 2 WEEK) <= date(updatetime) order by hot desc limit 0,7";		
...
{
	global $DH_html_path,$DH_index_url,$DH_html_url,$conn,$linkway,$movietype,$moviecountry,$linktype;	
	$DH_index_cats='';
	...
	foreach($movietype as $movietype_index=>$movietype_name)
	{
		...
		{	
			...
			$sql="select * from page where cattype = $movietype_index and catcountry = $moviecountry_index and trail=0 order by updatetime desc limit 0,5 ";
